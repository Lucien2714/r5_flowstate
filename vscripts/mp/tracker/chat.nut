...
void function RegisterAllChatCommands() //if chat commands enabled.
{
	...
	//Custom commands for CN server admins
	Commands_Register( "!kick", cmd_kick, [ "/kick", "\\kick" ] )
	Commands_Register( "!ban", cmd_ban, [ "/ban", "\\ban" ] )
	Commands_Register( "!unban", cmd_unban, [ "/unban", "\\unban" ] )
	Commands_Register( "!kicksay", cmd_kicksay, [ "/kicksay", "\\kicksay" ] )
	Commands_Register( "!bansay", cmd_bansay, [ "/bansay", "\\bansay" ] )
	Commands_Register( "!sayall", cmd_sayall, [ "/sayall", "\\sayall" ] )
...
void function cmd_kick( string tag, array<string> args, entity activator )
{
	args.remove( 0 ) //remove command tag
	array<string> kickArgs = [ "kick" ]
	foreach (arg in args) {
		kickArgs.append(arg)
	}
	ClientCommand_mkos_admin( activator, kickArgs )
}
void function cmd_ban( string tag, array<string> args, entity activator ) {
	args.remove( 0 ) //remove command tag
	array<string> banArgs = [ "ban" ]
	foreach (arg in args) {
		banArgs.append(arg)
	}
	ClientCommand_mkos_admin( activator, banArgs )
}
void function cmd_unban( string tag, array<string> args, entity activator )
{
	args.remove( 0 ) //remove command tag
	array<string> unbanArgs = [ "unban" ]
	foreach (arg in args) {
		unbanArgs.append(arg)
	}
	ClientCommand_mkos_admin( activator, unbanArgs )
}
void function cmd_kicksay( string tag, array<string> args, entity activator )
{
	args.remove( 0 ) //remove command tag
	array<string> kicksayArgs = [ "kicksay" ]
	foreach (arg in args) {
		kicksayArgs.append(arg)
	}
	ClientCommand_mkos_admin( activator, kicksayArgs )
}
void function cmd_bansay( string tag, array<string> args, entity activator )
{
	args.remove( 0 ) //remove command tag
	array<string> bansayArgs = [ "bansay" ]
	foreach (arg in args) {
		bansayArgs.append(arg)
	}
	ClientCommand_mkos_admin( activator, bansayArgs )
}
void function cmd_sayall( string tag, array<string> args, entity activator )
{
	args.remove( 0 ) //remove command tag
	array<string> sayallArgs = [ "sayall" ]
	foreach (arg in args) {
		sayallArgs.append(arg)
	}
	ClientCommand_mkos_admin( activator, sayallArgs )
}
